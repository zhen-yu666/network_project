...
main(int argc, char* argv[]) {
  ...
  while(true)  // 事件循环。
  {
    ...
    {
      if(evs[ii].data.fd
         == listenfd)  // 如果是listenfd有事件，表示有新的客户端连上来。
      {
        ////////////////////////////////////////////////////////////////////////
      ...
      } else  // 如果是客户端连接的fd有事件。
      {
        ...
        else if(evs[ii].events
                & (EPOLLIN | EPOLLPRI))  // 接收缓冲区中有数据可以读。
        {
          char buffer[1024];
          while(
